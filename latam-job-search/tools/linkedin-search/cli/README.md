...
```bash
cd <skill-dir>/tools/linkedin-search/cli
...
bun run cli/src/cli.ts search -q "backend engineer" -l "Hyderabad, Telangana, India" --jobage 7 --format table

# Design roles in London
bun run cli/src/cli.ts search -q "product designer" -l "London, United Kingdom" --format table

# Fully remote
bun run cli/src/cli.ts search -q "technical writer" -l "Remote" --remote remote --format table

# Full detail for one job
bun run cli/src/cli.ts detail 4426311357 --format plain
```
